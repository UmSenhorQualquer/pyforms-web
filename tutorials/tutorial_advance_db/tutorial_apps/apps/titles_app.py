...
from tutorial.models import Titles

class TitlesAdminApp(ModelAdminWidget):
    

    UID   = 'tutorial-Titles-app'.lower()
    MODEL = Titles
    
    TITLE = 'Titless'

    #list of filters fields
    #LIST_FILTER    = ['emp_no','title','from_date','to_date']

    #list of fields to display in the table
    #LIST_DISPLAY   = ['emp_no','title','from_date','to_date']
    
    #fields to be used in the search
    #SEARCH_FIELDS  = ['emp_no','title','from_date','to_date']
    
    #sub models to show in the interface
    #INLINES        = []
    
    #formset of the edit form
    #FIELDSETS      = ['emp_no','title','from_date','to_date']
    
    #read only fields
    #READ_ONLY      = ['emp_no','title','from_date','to_date']
    
    #EDITFORM_CLASS = TitlesModelFormWidget    #edit form class
